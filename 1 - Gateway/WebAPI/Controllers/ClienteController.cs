...
using Application.ClienteHandler;
...
namespace API.WebApi.Controllers {
    ...
    public class ClienteController(UserHandler userHandler, ClienteHandler handler, INotification notification) : BaseController(userHandler, notification) {
        // Endpoint para buscar todos os registros com base em um RequestAllDto via POST
        [HttpPost("/cliente/getAll")]
        public Task<ActionResult> GetAll(RequestAllClienteDto request) => Post(request, handler.GetAll);
        ...
        [HttpPost("/cliente/upsert")]
        public Task<ActionResult> Upsert(ClienteDto crud) => Post(crud, r => handler.Upsert(r, CurrentUser));
