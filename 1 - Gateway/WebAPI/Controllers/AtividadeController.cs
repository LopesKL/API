...
using Application.AtividadeHandler;
...
namespace API.WebApi.Controllers {
    ...
    public class AtividadeController(UserHandler userHandler, AtividadeHandler handler, INotification notification) : BaseController(userHandler, notification) {
        // Endpoint para buscar todos os registros com base em um RequestAllDto via POST
        [HttpPost("/atividade/getAll")]
        public Task<ActionResult> GetAll(RequestAllAtividadeDto request) => Post(request, handler.GetAll);

        // Endpoint para buscar um registro com base em um ID via GET
        [HttpGet("/atividade/getById/{id}")]
        ...
        [HttpPost("/atividade/upsert")]
        public Task<ActionResult> Upsert(AtividadeDto atividade) => Post(atividade, r => handler.Upsert(r, CurrentUser));

        // Endpoint para deletar um registro com base no ID fornecido via DELETE
        [HttpDelete("/atividade/{id}")]
